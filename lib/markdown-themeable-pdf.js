    ...
    convertFile: function (filePath, encoding) {
        ...
        fs.readFile(filePath, encoding, function (err, markdown) {
            ...
            if (jobInfo.exportType == 'html') {
                ...
                const tmpPath = path.join(remote.app.getPath('temp'), jobInfo.destFileBase + '.html');

                fs.writeFile(tmpPath, dom, function (error) {
                    if (error)
                        throw error;
                    ...
                    win.webContents.on('did-finish-load', () => {
                        win.webContents.printToPDF({
                            ...
                            fs.writeFile(jobInfo.destFile, data, (error) => {
                                if (error)
                                    throw error;
